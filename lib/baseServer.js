...
const { loadEnvFile, loadEnvFiles } = require("zyx-env");
...
class BaseServer extends Base {
  ...
  constructor(
    serverConfigFile,
    tenantConfigFilemask,
    encdryptKey,
    options = {}
  ) {
    const serverConfig = loadEnvFile(
      serverConfigFile,
      encdryptKey,
      undefined,
      options
    ...
    this.#tenantConfigs = loadEnvFiles(
      tenantConfigFilemask,
      encdryptKey,
      undefined,
      options
    ...
    // initialize internal array of BaseRoute class instances
    this.#routes = [];

    // initialize array of values to cleanup
    this.#closeStack = [];

    //call method to perform all initialization of express features
    this.initExpress();
  }
