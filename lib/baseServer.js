...
class BaseServer {
  ...
  #routes;
  #closeStack;
  #domainTenantMap; // ✅ Declare private field to fix error
  ...
  router(leadPath, routerClass) {
    if (typeof routerClass !== "function") {
      throw new Error(
        `Router "${name}" must be a class or constructor function`
      );
    }
  ...
  initTenants() {
    // Normalize tenants and store them
    this.#tenants = this.#tenantConfigs.map(config => {
      // Parse comma-separated domains, trim and filter empty ones
      ...
      return {
        id: config.id,
        node: config.node,
        domains, // store array of domains
        config,
        models: {},
      };
    });

    // Create a fast lookup map: domain -> tenant
    ...
    this.#express.use((req, res, next) => {
      ...
      // Set site_ config values to res.locals
      res.locals = {};
