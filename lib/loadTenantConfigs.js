...
function loadTenantConfigs() {
  return loadEnvFiles(
    ...
    new Schema({
      ...
      site_author_name: stringType({ min: 1, max: 255, required: true }),
      site_author_url: stringType({ min: 1, max: 255, required: true }),
      site_copyright: integerType({ min: 2025, max: 2100, required: true }),
